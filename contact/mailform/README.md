...
├── mailform.js          クライアント側（HTML属性ベース検証 + Ajax送信 + reCAPTCHA）→ assets/js/ へ移動
├── form.sample.html     フォームマークアップの参考例
├── thanks.sample.html   完了ページの雛形
└── vendor/phpmailer/    PHPMailer 6.9.3（バージョン固定・変更不要）
```

`mailform.js` は設置時に `assets/js/` へ**移動**する（`mailform/` 側に重複を残さない）。`*.sample.*` と `README.md` は参考用でサーバーには不要。

**サーバーにアップするのは** `submit.php`・`config.php`・`vendor/`・`outbox/`（書き込み可）だけ。参考ファイルは公開ディレクトリに置かない。

## クライアント側（mailform.js）

`.js-mailform` フォームに対して、HTML 属性から検証ルールを読む（`required` / `type` / `maxlength` / `pattern`）。ルールを JS に書かないので、マークアップを変えるだけで案件をまたいで使える。クラスは「掴み=`js-` / 状態=`is-` / 見た目=`c-`」で役割分離（規約の正は `form.sample.html` の冒頭コメント）。JS 無効時は素の POST で `submit.php` → thanks リダイレクトにフォールバックする。
